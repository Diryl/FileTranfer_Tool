...
#ifndef __CPLUSFRAME_H
#define __CPLUSFRAME_H 1
...
// logfile：用于记录错误的日志文件的指针，如果为0，发生了错误不记录日志。
// 返回值：true-成功；false-发送失败，失败的原因有两种：1）sockfd不可用，2）待发送的文件不存在或权限不足。
bool SendDirFile(int sockfd,struct st_fileinfo *stfileinfo,CLogFile *logfile=0);
...
// logfile：用于记录错误的日志文件的指针，如果为0，发生了错误不记录日志。
// 返回值：true-成功；false-接收失败，失败的原因有两种：1）sockfd不可用，2）待发送的文件不存在或权限不足。
bool RecvDirFile(int sockfd,struct st_fileinfo *stfileinfo,char *filepath,CLogFile *logfile=0);
