...
// 获取文件路径名filepathname中的文件名filename
// filepathname：待获取的文件路径名的字符串
// filename：获取的文件名
void GetFileName(const char* filepathname, char* filename){
  if(filepathname == 0 || filename == 0) {return;}
  char _filepathname[301];
  memset(_filepathname, 0, sizeof(filepathname));
  strcpy(_filepathname, filepathname);
  char* strp = strrchr(_filepathname, '/');
  strcpy(filename, strp+1);
  return;
}
...
// 把文件通过sockfd发送给对端
bool SendDirFile(int sockfd,struct st_fileinfo *stfileinfo,CLogFile *logfile)
{
  char strSendBuffer[301],strRecvBuffer[301];
  memset(strSendBuffer,0,sizeof(strSendBuffer));

  char filename[301];
  GetFileName(stfileinfo->filename, filename);

  snprintf(strSendBuffer,300,"<filename>%s</filename><filesize>%d</filesize><mtime>%s</mtime>",filename,stfileinfo->filesize,stfileinfo->mtime);

  if (TcpWrite(sockfd,strSendBuffer) == false)
  {
    if (logfile!=0) logfile->Write("Send File(%s) Info failed.\n", stfileinfo->filename); 
    return false;
  }
  if (logfile!=0) logfile->Write("Send File(%s) Info success.\n", stfileinfo->filename);
  ...
  if ( (File.Open(stfileinfo->filename,"rb")) == 0 )
  {
    if (logfile!=0) logfile->Write("File.Open(%s) failed.\n",stfileinfo->filename); 
    return false;
  }
  if (logfile!=0) logfile->Write("File.Open(%s) success.\n", stfileinfo->filename);
  ...
  while (true)
  {
    ...
    bytes=File.Fread(buffer,onread);
  ...
  if(logfile!=0) logfile->Write("File(%s) has sended, and has deleted.\n", stfileinfo->filename);
  ...
  if (TcpRead(sockfd,strRecvBuffer,&buflen)==false)
  {
    if (logfile!=0) logfile->Write("TcpRead(%s) failed.\n"); 
    return false;
  }

  if (strcmp(strRecvBuffer,"ok")!=0) return false;

  if(logfile!=0) logfile->Write("File(%s) server received.\n",stfileinfo->filename);
...
// 接收通过socdfd发送过来的文件
bool RecvDirFile(int sockfd,struct st_fileinfo *stfileinfo,char *filepath, CLogFile *logfile)
{
  char strSendBuffer[301];
  char strRecvBuffer[301];

  memset(strRecvBuffer, 0, sizeof(strRecvBuffer));
  int buflen=0;
  if (TcpRead(sockfd,strRecvBuffer,&buflen)==false)
  {
    if (logfile!=0) logfile->Write("TcpRead(fileinfo) failed.\n"); 
    return false;
  }

  if (logfile!=0) logfile->Write("TcpRead(%s) success.\n",strRecvBuffer);  

  char _filename[301];
  memset(_filename, 0, sizeof(_filename));

  if(GetXMLBuffer(strRecvBuffer, "filename", _filename, 300) == false) {logfile->Write("Get filename failed.\n");}
  if(GetXMLBuffer(strRecvBuffer, "filesize", &stfileinfo->filesize) == false) {logfile->Write("Get filesize failed.\n");}
  if(GetXMLBuffer(strRecvBuffer, "mtime", stfileinfo->mtime) == false) {logfile->Write("Get file mtime failed.\n");}

  sprintf(stfileinfo->filename, "%s/%s", filepath, _filename); 
  if (logfile!=0) logfile->Write("File receive info: filename: %s, filesize: %d, filemtime: %s\n", stfileinfo->filename, stfileinfo->filesize, stfileinfo->mtime);

  CFile File;

  if ( (File.OpenForRename(stfileinfo->filename,"wb")) ==0)     // FOPEN可创建目录
  {
    if (logfile!=0) logfile->Write("File.OpenForRename(%s) failed.\n", stfileinfo->filename); 
    return false;
  }
  if (logfile!=0) logfile->Write("File.OpenForRename(%s) success.\n", stfileinfo->filename);
  ...
  if (logfile!=0) logfile->Write("File(%s) has received.\n", stfileinfo->filename);

  memset(strSendBuffer,0,sizeof(strSendBuffer));

  if (File.CloseAndRename()==true) strcpy(strSendBuffer,"ok");
  else strcpy(strSendBuffer,"failed");
  if (logfile!=0 && strcmp(strSendBuffer, "ok") == 0) logfile->Write("File(%s) has renamed.\n", stfileinfo->filename);

  // 重置文件的时间
  UTime(stfileinfo->filename, stfileinfo->mtime);  
  if (logfile!=0) logfile->Write("File(%s) has modify time.\n", stfileinfo->filename);  

  // 向对端返回响应内容
  if (TcpWrite(sockfd,strSendBuffer)==false)
  {
    if (logfile!=0) logfile->Write("TcpWrite(%s) failed.\n", strSendBuffer); 
    return false;
  }

  if (strcmp(strSendBuffer,"ok") != 0) return false;

  if (logfile!=0) logfile->Write("TcpWrite(%s) success.\n", strSendBuffer);
